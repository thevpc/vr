            ...
            if (user != null) {
                ...
                if (user != null) {
                    if (!user.isEnabled()) {
                        ...
                        trace.trace("impersonate", "successfull impersonate of " + login + ". but password " + password + " seems not to be correct", login, getClass().getSimpleName(), null, null, s.getUser().getLogin(), s.getUser().getId(), Level.WARNING, s.getClientIpAddress());
                    }
                } else {
                    trace.trace(
