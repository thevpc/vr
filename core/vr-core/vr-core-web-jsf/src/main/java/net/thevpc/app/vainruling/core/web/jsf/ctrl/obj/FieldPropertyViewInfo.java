...
public class FieldPropertyViewInfo {
    ...
    private FieldPropertyViewInfo(Field field, DataType dataType, Map<String, Object> configuration) {
        ...
        VrAccessMode m = editorCtrl.getModel().getMode();
        listMode = m == VrAccessMode.READ;
        insertMode = m == VrAccessMode.PERSIST;
        updateMode = m == VrAccessMode.UPDATE || m == VrAccessMode.BULK_UPDATE;
        updateAllMode = m == VrAccessMode.BULK_UPDATE;
        if (field != null) {
            AccessLevel effectiveAccessLevel = field.getEffectiveAccessLevel(m.toUpaMode());
