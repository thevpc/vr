...
import net.thevpc.app.vainruling.plugins.academic.service.util.DefaultTeacherPeriodFilter;
...
import net.thevpc.app.vainruling.plugins.academic.service.stat.StatCache;
...
import net.thevpc.app.vainruling.plugins.academic.service.stat.GlobalStat;
import net.thevpc.app.vainruling.plugins.academic.service.util.TeacherFilterFactory;
...
import net.thevpc.app.vainruling.plugins.academic.service.util.CourseAssignmentFilterAnd;
...
//        getOthersCourseFilter().onChangePeriod();
    ...
    public void onRefresh() {
        ...
        List<GlobalStatByCourseCore> globalStatByCourseCore = new ArrayList<>();
        globalStatByCourseCore.add(new GlobalStatByCourseCore(null, allTeachers));
        for (AcademicCourseCore _item : p.findAcademicCourseCores(periodId <= 0 ? null : periodId,null)) {
            globalStatByCourseCore.add(new GlobalStatByCourseCore(_item,
                    p.evalGlobalStat(periodId <= 0 ? -100 : periodId,
                            TeacherFilterFactory.and(
                                    TeacherFilterFactory.custom().addAcceptedCourseCore(_item.getId()),
                                    teacherFilter
                            ), 
                            new CourseAssignmentFilterAnd().and(courseAssignmentFilter).and(
                                    new DefaultCourseAssignmentFilter().assignmentCourseCore().addAccepted(_item).build()
                            ),
                            deviationConfig, null)
            ));
        }
        getModel().setGlobalStatByCourseCore(globalStatByCourseCore);
...
//                    setValuePercent(pie, category, npv / s);
    ...
    public List<GlobalStatByCourseCore> getGlobalStatByCourseCores() {
        List<GlobalStatByCourseCore> all = new ArrayList<>();
        DefaultTeacherPeriodFilter d = getTeacherFilter().getTeacherFilter();
        for (GlobalStatByCourseCore globalStatByCourseCore : getModel().getGlobalStatByCourseCores()) {
            if (globalStatByCourseCore.getCourseCore() == null || d.acceptCourseCore(globalStatByCourseCore.getCourseCore())) {
                all.add(globalStatByCourseCore);
            }
        }
        return all;
    }
    ...
    public List<AcademicTeacherSituation> getGlobalStatSituationsAndCourseCore() {
        List<AcademicTeacherSituation> list = new ArrayList<>();
        Set<Integer> set = new HashSet<>();
        DefaultTeacherPeriodFilter teacherFilter = getTeacherFilter().getTeacherFilter();
        for (GlobalStatByCourseCore globalStatByCourseCore : getGlobalStatByCourseCores()) {
            for (GlobalAssignmentStat s : globalStatByCourseCore.getStat().getSituationDetails(null, null)) {
                AcademicTeacherSituation sit = s.getSituation();
                if (sit != null && !set.contains(sit.getId())) {
                    if (s.getTeachersCount() > 0) {
                        if (teacherFilter.acceptTeacherSituation(sit)) {
    ...
    public class Model {
        ...
        public void setGlobalStatByDisciplines(List<GlobalStatByDiscipline> list) {
            this.globalStatByDisciplines = list;
        }

        public void setGlobalStatByCourseCore(List<GlobalStatByCourseCore> list) {
            this.globalStatByCourseCore = list;
        }
    ...
    public static class GlobalStatByCourseCore {

        private GlobalStat stat;
        private AcademicCourseCore courseCore;

        public GlobalStatByCourseCore(AcademicCourseCore courseCore, GlobalStat stat) {
            this.stat = stat;
        ...
        public AcademicCourseCore getCourseCore() {
            return courseCore;
        }

        public void setCourseCore(AcademicCourseCore courseCore) {
            this.courseCore = courseCore;
        }
    }
}
