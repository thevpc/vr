...
//        StatCache.PeriodCache periodCache = cache.forPeriod(periodId);
        KeyValueList<Integer, AcademicCourseAssignment> courseAssignments = academic.findCourseAssignments(periodId);
...
//                cache);
        ...
        KeyValueList<Integer, TeacherPeriodStat> all = evalTeacherStatList(periodId, null, courseAssignmentFilter, deviationConfig, monitor);
...
//        log.log(Level.INFO,"updateCoursePlanValuesByLoadValues in {1}",new Object[]{ch.stop()});
    ...
    public GlobalStat evalGlobalStat(int periodId, TeacherPeriodFilter teacherFilter, CourseAssignmentFilter filter, DeviationConfig deviationConfig, ProgressMonitor mon) {
        ...
        KeyValueList<Integer, TeacherPeriodStat> ts = evalTeacherStatList(periodId, teacherFilter, filter, deviationConfig, mons[0]);
        int i = 0;
        int max = ts.size();
...
//        StatCache.PeriodCache periodCache = cache.forPeriod(periodId);
        ...
        for (AcademicCourseAssignment value : courseAssignments) {
            ...
            if (permanent) {
                ...
                AutoValueMap<String, DisciplineStat> nonOfficialDisciplines = s.getPermanentLoadValueByNonOfficialDiscipline();
                ...
                officialDiscipline.getValue().add(loadValue);
                AutoValueMap<String, DisciplineStat> nonOfficialDisciplines = s.getNonPermanentLoadValueByNonOfficialDiscipline();
                Set<String> ood = getContext().getPlugin().parseDisciplinesNames(value.getCoursePlan().getDiscipline(), false);
                if (ood.isEmpty()) {
                    ood.add("");
                }
...
//        );
    ...
    public KeyValueList<Integer, TeacherPeriodStat> evalTeacherStatList(final int periodId, TeacherPeriodFilter teacherFilter, CourseAssignmentFilter courseAssignmentFilter, DeviationConfig deviationConfig, ProgressMonitor mon) {
        ...
        KeyValueList<Integer, TeacherPeriodStat> stats = Collections2.keyValueList(teacherPeriodStatMapListConverter);
    ...
    public List<TeacherSemesterStat> evalTeacherSemesterStatList(int periodId, Integer semesterId, TeacherPeriodFilter teacherFilter, CourseAssignmentFilter filter, DeviationConfig deviationConfig, ProgressMonitor mon) {
        ...
        KeyValueList<Integer, TeacherPeriodStat> teacherPeriodStats = evalTeacherStatList(periodId, teacherFilter,
    ...
    private KeyValueList<Integer, AcademicCoursePlan> extractCourses(List<AcademicCourseAssignment> all) {
        KeyValueList<Integer, AcademicCoursePlan> m = Collections2.keyValueList(
