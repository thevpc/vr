...
//    private static String file_result = "${user.home}/Data/eniso/students/2015-2016/choix-options/IA/2IA-OPTIONS-FINAL.xlsx";
    ...
    public static final String USER_HOME_DATA_ENISO_STUDENTS_2015_2016 = "${user.home}/Data/eniso/students/2016-2017/choix-options";
    ...
    private static Comparator<StudentInfo> STUDENT_MERITE_COMPARATOR = new Comparator<StudentInfo>() {
        ...
        public int compare(StudentInfo o1, StudentInfo o2) {
            ...
            double moyg1 = evalScore(o1);
            double moyg2 = evalScore(o2);
    ...
    private static double evalScore(StudentInfo o1) {
...
//                        studentInfo.rank = ((""+values[6]).isEmpty()) ? -1: asInt(values[6]);
                        studentInfo.rattr = String.valueOf(values[col++]).equals("R");
                        studentInfo.master = asDouble(values[col++]);
                        String s = String.valueOf(values[col++]);
                        studentInfo.redoublement = s.equals("1");
            ...
            for (StudentInfo s : liste) {
                ...
                for (int i = 0; i < nbOptions; i++) {
                    if (s.choice[i] > 0) {
                        ...
                        if (cc.available > 0) {
                            s.assignment = s.choice[i];
                            cc.available--;
//                            int c = cc.available + getAssignements(liste, cc.number);
                            choiceFound = true;
                            System.out.println("Assignment " + (i + 1) + " for " + s + "  ==> " + cc);
                            break;
                        }
                    }
                }
            ...
            for (StudentInfo studentInfo : liste) {
                for (int i = 0; i < studentInfo.choice.length; i++) {
                    if(studentInfo.assignment==studentInfo.choice[i]){
                        studentInfo.indexChoice=i+1;
                    }
                }
            }
            SheetFormatter frm = pfm.createSheetFormatter(new java.io.File(file_result.replace("${user.home}",System.getProperty("user.home"))));
            for (String c : new String[]{"NUM","ID", "NOM", "PRENOM", "MOYG", "MOYR", "MASTER", "RANG", "Score", "RDL","RATTR", "CHOIX1", "CHOIX2", "CHOIX3", "AFFECTATION","SATISFACTION"}) {
                SheetColumn cc = new SheetColumn();
                cc.setName(c);
                frm.getColumns().add(cc);
            }
