...
import net.thevpc.app.vainruling.core.web.jsf.ctrl.dialog.ProfileExprDialogCtrl;
...
import net.thevpc.app.vainruling.plugins.inbox.model.MailboxSentAttachment;
import net.thevpc.app.vainruling.plugins.inbox.model.MailboxMessageFormat;
import net.thevpc.app.vainruling.plugins.inbox.model.MailboxSent;
...
public class MailboxCtrl implements VrPageInfoResolver, VrMenuProvider {
    ...
    public void onNew() {
        ...
        resetNewMessage(item,VrAccessMode.PERSIST);
    }

    private void resetNewMessage(MailboxSent item,VrAccessMode mode){
...
//    }
    ...
    public void onReply() {
        ...
        if (current != null && current.isInbox()) {
            ...
            resetNewMessage(s,VrAccessMode.PERSIST);
    ...
    public boolean isNewOrUpdateMode() {
        try {
            return getModel().getMode() == VrAccessMode.PERSIST || getModel().getMode() == VrAccessMode.UPDATE;
        } catch (RuntimeException ex) {
            log.log(Level.SEVERE, "Error", ex);
            throw ex;
        }
